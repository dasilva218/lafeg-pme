...
const offresFinancement = [
  ...
  },
  {
    ...
    documentation: "Demande client détaillée, États financiers (2-3 ans), PV du comité",
  ...
  },
  {
    ...
    devise: "XAF",
    admissibilite: "Compte courant, Cotisation tontine (3 mois min), Activité génératrice de revenus",
...
const toggleOffreSelection = (offre: OffreFinancement) => {
  ...
  const [selectedOffre, setSelectedOffre] = useState<OffreFinancement | null>(null)
  ...
  const filteredOffres = useMemo(() => {
    return offresFinancement.filter((offre) => {
      ...
      const matchesSecteur = filterSecteur === "all" || offre.secteur.includes(filterSecteur)
  ...
  const secteurs = [...new Set(offresFinancement.flatMap((offre) => offre.secteur.split(", ")))]
...
const formatMontant = (min: string, max: string, devise: string): string => {
    if (min === "Variable" || min === "Selon cotisation" || min === "Selon opportunité") {
        return min
    }
          ...
          <Card className="mb-8 border-[#063a1e] border-2">
            <CardHeader className="bg-[#063a1e] text-white">
              <CardTitle className="flex items-center justify-between">
                ...
                <Button variant="secondary" size="sm" onClick={() => setSelectedOffres([])}>
                  Vider la sélection
                </Button>
              </CardTitle>
            </CardHeader>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      ...
                      {selectedOffres.map((offre) => (
                        <th key={offre.id} className="text-left p-2 min-w-[200px]">
                          {offre.nom}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    ...
                    <tr className="border-b">
                      ...
                      {selectedOffres.map((offre) => (
                        <td key={offre.id} className="p-2">
                          {formatMontant(offre.montantMin, offre.montantMax, offre.devise)}
                        </td>
                      ))}
                    </tr>
                    <tr className="border-b">
                  ...
                  {filteredOffres.map((offre) => (
                    <TableRow key={offre.id} className="hover:bg-gray-50">
                      <TableCell className="font-medium">
                        <div>
                          <p className="font-semibold text-[#063a1e]">{offre.nom}</p>
                          <p className="text-sm text-gray-600 truncate max-w-xs">{offre.but.substring(0, 80)}...</p>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-col">
                          <span className="font-medium">{offre.fournisseur}</span>
                      ...
                      <TableCell>
                        <span className="text-sm font-medium">{offre.tauxInteret}</span>
                      </TableCell>
                      <TableCell>
                        <div className="flex gap-2">
                          <Dialog>
                            <DialogTrigger asChild>
                              <Button variant="outline" size="sm" onClick={() => setSelectedOffre(offre)}>
                            ...
                            <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                              ...
                              {selectedOffre && (
                                <div className="space-y-6">
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                      ...
                                      <div className="space-y-2 text-sm">
                                        <p>
                                          <strong>Fournisseur:</strong> {selectedOffre.fournisseur}
                                        </p>
                                        <p>
                                          <strong>Type:</strong>{" "}
                                          <Badge className={getTypeColor(selectedOffre.type)}>
                                            {selectedOffre.type}
                                          </Badge>
                                        </p>
                                        <p>
                                          <strong>Secteur:</strong> {selectedOffre.secteur}
                                        </p>
                                        <p>
                                          <strong>Taux d'intérêt:</strong> {selectedOffre.tauxInteret}
                                        </p>
                                      </div>
                                    </div>
                                    <div>
                                      <h4 className="font-semibold mb-2">Conditions financières</h4>
                                      <div className="space-y-2 text-sm">
                                        <p>
                                        ...
                                        <p>
                                          <strong>Durée de remboursement:</strong> {selectedOffre.remboursement}
                                        </p>
                                      </div>
                                    </div>
                                  </div>
                                  ...
                                  <div>
                                    <h4 className="font-semibold mb-2">Objectif du financement</h4>
                                    <p className="text-sm text-gray-700">{selectedOffre.but}</p>
                                  </div>

                                  <div>
                                    <h4 className="font-semibold mb-2">Conditions d'admissibilité</h4>
                                    <p className="text-sm text-gray-700">{selectedOffre.admissibilite}</p>
                                  </div>

                                  <div>
                                    <h4 className="font-semibold mb-2">Documentation requise</h4>
                                    <p className="text-sm text-gray-700">{selectedOffre.documentation}</p>
                                  </div>
                          ...
                          <Button
                            variant={selectedOffres.find((o) => o.id === offre.id) ? "default" : "outline"}
                            size="sm"
                            onClick={() => toggleOffreSelection(offre)}
                            disabled={!selectedOffres.find((o) => o.id === offre.id) && selectedOffres.length >= 3}
                          >
                            <Plus className="w-4 h-4 mr-1" />
                            {selectedOffres.find((o) => o.id === offre.id) ? "Sélectionné" : "Comparer"}
                          </Button>
                        </div>
                      </TableCell>
                    </TableRow>
